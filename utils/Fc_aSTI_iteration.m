function [chi_MMS,chi_MSA,Pev,Offset]= Fc_aSTI_iteration(tissue_phase,H_Matrix,mask,offset_mask,iter_num,outpath)
%====================================
...
%% step 1: Solving ten unknowns using lsqr.
lsqr_tol = 0.001;                            % LSQR tolerance
lsqr_iter = 30;
[lsqr_out, ~, ~, ~] = lsqr(@afun, tissue_phase(:), lsqr_tol, lsqr_iter);
out=reshape(lsqr_out,[sizeVol,10]);

%% step 2: Estimate MSA and PEV
chi_tensor=out(:,:,:,1:9);
[symmpart,~]=tensor_decomp(chi_tensor);
[Chi_vec,~,~,chi_MSA]=Cal_eig(symmpart,mask,'asymm');
Pev=Chi_vec(:,:,:,:,3);
save_nii(make_nii(chi_MSA),fullfile(outpath,'MSA_Fc-aSTI_iter0.nii'));
save_nii(make_nii(abs(Pev)),fullfile(outpath,'absPEV_Fc-aSTI_iter0.nii')); 
save_nii(make_nii(Pev),fullfile(outpath,'PEV_Fc-aSTI_iter0.nii'));   
...
save_nii(make_nii(chi_MMS),fullfile(outpath,'MMS_Fc-aSTI_iter0.nii'));
save_nii(make_nii(Offset),fullfile(outpath,'Fres_Fc-aSTI_iter0.nii'));
...
if iter_num>0
    for cpt=1:iter_num
        ...
        [~,~,chi_MSA,Pev]=aSTI_inverse(phase_temp,H_Matrix,mask);
        save_nii(make_nii(chi_MSA),fullfile(outpath,['MSA_Fc-aSTI_iter',num2str(cpt),'.nii']));
        save_nii(make_nii(Pev),fullfile(outpath,['PEV_Fc-aSTI_iter',num2str(cpt),'.nii']));
        save_nii(make_nii(abs(Pev)),fullfile(outpath,['absPEV_Fc-aSTI_iter',num2str(cpt),'.nii']));
        ...
        [chi_MMS, Offset, flag, relres, iter, ~] = Inverse_fun_MMS_offset(Params, maxit, tol);
        disp(['Flag: ', num2str(flag), '  Relres: ', num2str(relres), '  Iter: ', num2str(iter)]);
         save_nii(make_nii(chi_MMS),fullfile(outpath,['MMS_Fc-aSTI_iter',num2str(cpt),'.nii']));
        save_nii(make_nii(Offset),fullfile(outpath,['Offset_Fc-aSTI_iter',num2str(cpt),'.nii']));    
    end
end
    ...
    function res = afun( in,tflag)
        if strcmp(tflag,'transp')
            ...
            Res = zeros([sizeVol, 10]);
            for n = 1:N_direction
                Res(:,:,:,1) = Res(:,:,:,1) + real(ifftn(Darray(:,:,:,1,n) .*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,2) = Res(:,:,:,2) +real(ifftn( Darray(:,:,:,2,n) .*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,3) = Res(:,:,:,3) + real(ifftn(Darray(:,:,:,3,n).*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,4) = Res(:,:,:,4) +real(ifftn( Darray(:,:,:,4,n) .* fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,5) = Res(:,:,:,5) +real(ifftn( Darray(:,:,:,5,n).*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,6) = Res(:,:,:,6) +real(ifftn( Darray(:,:,:,6,n).* fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,7) = Res(:,:,:,7) +real(ifftn( Darray(:,:,:,7,n) .*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,8) = Res(:,:,:,8) +real(ifftn( Darray(:,:,:,8,n) .*fftn( im(:,:,:,n).*mask)));
                
                Res(:,:,:,9) = Res(:,:,:,9) +real(ifftn( Darray(:,:,:,9,n) .* fftn( im(:,:,:,n).*mask)));
              
                Res(:,:,:,10)=Res(:,:,:,10)+offset_mask.*im(:,:,:,n);
            end
            res = Res(:);
            
        else
            im = reshape(in, [sizeVol,10]);
            Res = zeros([sizeVol, N_direction]);
            
            for n = 1:N_direction               
                Res(:,:,:,n) =mask.*real(ifftn( Darray(:,:,:,1,n) .*fftn( im(:,:,:,1)) + ...
                    Darray(:,:,:,2,n).* fftn(im(:,:,:,2)) + ...   
                    Darray(:,:,:,3,n) .*fftn( im(:,:,:,3)) + ...  
                   Darray(:,:,:,4,n) .* fftn(im(:,:,:,4)) + ...   
                   Darray(:,:,:,5,n) .*fftn( im(:,:,:,5)) + ...               
                   Darray(:,:,:,6,n) .*fftn( im(:,:,:,6)) + ...  
                   Darray(:,:,:,7,n) .*fftn( im(:,:,:,7)) + ...    
                   Darray(:,:,:,8,n) .* fftn(im(:,:,:,8)) + ...    
                   Darray(:,:,:,9,n) .* fftn(im(:,:,:,9) )))+ ...  
                    offset_mask.*im(:,:,:,10);                                                                                            
            end
            res = Res(:);
            fprintf('+')
