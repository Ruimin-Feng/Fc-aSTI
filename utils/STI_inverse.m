function [chi_tensor,chi_MMS,chi_MSA,Pev]=STI_inverse(phase_tissue,H_Matrix,mask,varargin)
%====================================
% This function is to solve the conventional STI
...
parfor loopvar = 1:poolnum
    warning off all
    [respar{loopvar}, flag, relres, iter] = lsqr(@STI_fun, thetai0par{loopvar}, lsqr_tol, lsqr_iter, [], [], [], H_Matrix,kxpar{loopvar},kypar{loopvar},kzpar{loopvar},k2par{loopvar},N_direction);  
    disp(['Solving STI---Flag: ', num2str(flag), '  Relres: ', num2str(relres), '  Iter: ', num2str(iter)])
end
toc
...
[Chi_vec,~,chi_MMS,chi_MSA]=Cal_eig(chi_tensor,mask,'symm');
Pev=Chi_vec(:,:,:,:,3);
if nargin>3
    try
        save_nii(make_nii(chi_MMS),fullfile(varargin{1},'MMS_STI.nii'));
        save_nii(make_nii(chi_MSA),fullfile(varargin{1},'MSA_STI.nii'));
        save_nii(make_nii(abs(Pev)),fullfile(varargin{1},'absPEV_STI.nii')); 
        save_nii(make_nii(Pev),fullfile(varargin{1},'PEV_STI.nii'));   
    catch
